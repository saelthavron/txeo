...
template <typename T>
void TensorAgg<T>::verify_parameters(const txeo::Tensor<T> &tensor,
...
txeo::Tensor<T> TensorAgg<T>::accumulate(const txeo::Tensor<T> &tensor, size_t axis,
  ...
  txeo::Tensor<T> resp(shape_resp, vec_resp);
...
txeo::Tensor<size_t> TensorAgg<T>::count(const txeo::Tensor<T> &tensor, size_t axis,
  ...
  txeo::Tensor<size_t> resp(shape_resp, vec_resp);
...
template <typename T>
txeo::Tensor<T> TensorAgg<T>::reduce_prod(const txeo::Tensor<T> &tensor,
                                          const std::vector<size_t> &axes) {
  TensorAgg<T>::verify_parameters(tensor, axes);
  try {
    auto resp = txeo::detail::TensorHelper::reduce_tensor<T>(
...
template <typename T>
txeo::Tensor<T> TensorAgg<T>::reduce_mean(const txeo::Tensor<T> &tensor,
                                          const std::vector<size_t> &axes) {
  TensorAgg<T>::verify_parameters(tensor, axes);
  try {
    auto resp = txeo::detail::TensorHelper::reduce_tensor<T>(
...
template <typename T>
txeo::Tensor<T> TensorAgg<T>::reduce_max(const txeo::Tensor<T> &tensor,
                                         const std::vector<size_t> &axes) {
  TensorAgg<T>::verify_parameters(tensor, axes);
  try {
    auto resp = txeo::detail::TensorHelper::reduce_tensor<T>(
...
template <typename T>
txeo::Tensor<T> TensorAgg<T>::reduce_min(const txeo::Tensor<T> &tensor,
                                         const std::vector<size_t> &axes) {
  TensorAgg<T>::verify_parameters(tensor, axes);
  try {
    auto resp = txeo::detail::TensorHelper::reduce_tensor<T>(
...
template <typename T>
txeo::Tensor<T> TensorAgg<T>::reduce_all(const txeo::Tensor<T> &tensor,
                                         const std::vector<size_t> &axes)
...
template <typename T>
txeo::Tensor<T> TensorAgg<T>::reduce_any(const txeo::Tensor<T> &tensor,
                                         const std::vector<size_t> &axes)
  requires(std::convertible_to<T, bool>)
{
  TensorAgg<T>::verify_parameters(tensor, axes);
...
template <typename T>
txeo::Tensor<T> TensorAgg<T>::reduce_variance(const txeo::Tensor<T> &tensor, size_t axis) {
  return TensorAgg<T>::accumulate(
...
template <typename T>
txeo::Tensor<T> TensorAgg<T>::reduce_median(const txeo::Tensor<T> &tensor, size_t axis) {
  return TensorAgg<T>::accumulate(
...
size_t TensorAgg<T>::count_non_zero(std::vector<T> &values) {
  return std::ranges::count_if(values,
                               [](const T &item) -> bool { return !txeo::detail::is_zero(item); });
}
