...
int main() {
  ...
  // ======================================================================================
  {
    ...
    try {
      auto data = txeo::MatrixIO::read_textfile<float>("data.csv", // Assume this file exists
                                                       ',',
      ...
      std::cout << "CSV Tensor shape: " << data.shape() << "\n"
                ...
                << "\n";
    } catch (const txeo::MatrixIOError &e) {
  ...
  // ======================================================================================
  {
    ...
    txeo::MatrixIO reader("data.tsv", '\t'); // Tab-separated
  ...
  {
    std::cout << "\n=== 3. Writing CSV ===\n";
    txeo::Matrix<int> matrix{{1, 2, 3}, {4, 5, 6}, {7, 8, 9}};

    txeo::MatrixIO::write_textfile(matrix, "output_matrix.csv",
                                   ',' // CSV format
    );
  ...
  {
    std::cout << "\n=== 4. Precision Writing ===\n";
    txeo::Matrix<double> measurements{{std::numbers::pi, std::numbers::e},
                                      {std::numbers::phi, std::numbers::egamma}};

    txeo::MatrixIO::write_textfile(measurements,
                                   4, // 4 decimal places
                                   "precision_data.csv");
  ...
  // ======================================================================================
  {
    ...
    txeo::Matrix<float> sensor_data{{25.4f, 18.9f, 30.1f}, {22.5f, 19.8f, 28.7f}};

    txeo::MatrixIO writer("sensor_data.psv", '|');
    try {
  ...
  // ======================================================================================
  {
    std::cout << "\n=== 6. Error Handling ===\n";
    try {
      auto invalid = txeo::MatrixIO::read_textfile<int>("non_existent_file.dat",
                                                        '$', // Invalid separator
                                                        true);
    } catch (const txeo::MatrixIOError &e) {
